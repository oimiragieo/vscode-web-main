...
```css
.login-form { }                    /* Class selector */
.form-input { }                    /* Class selector */
.submit-button:hover { }           /* Pseudo-class */
```

Concerning (Higher specificity):
```css
/* Line 31-43 in login.css */
.login-form > .field > .password { }  /* 3-level child combinator */
.login-form > .field > .submit { }    /* 3-level child combinator */

/* Line 227 in modern-login.css */
.form-input:hover { }              /* Good specificity */

/* Line 376 in modern-login.css */
.submit-button:hover .button-icon { }  /* Descendant combinator - acceptable */
```
...
<style>
  ...
  .login-card { background: var(--surface-elevated); }
  .form-input { width: 100%; padding: var(--spacing-3); }
</style>
...
document.addEventListener('keydown', (e) => {
  if (e.key === 'Escape' && loadingOverlay && 
...
<!-- Better: -->
<link rel="apple-touch-icon" sizes="192x192" 
      href="{{CS_STATIC_BASE}}/src/browser/media/pwa-icon-192.webp" />
<link rel="apple-touch-icon" sizes="192x192" 
      href="{{CS_STATIC_BASE}}/src/browser/media/pwa-icon-192.png" />
```
...
```typescript
// In routes/login.ts
const loginTemplate = req.args['use-modern-login'] 
  ? 'modern-login.html' 
  : 'login.html';
```
...
```html
<head>
  <meta charset="utf-8" />                    <!-- GOOD: First -->
  <meta name="viewport" ... />                <!-- GOOD: Before CSS -->
  <meta http-equiv="Content-Security-Policy"> <!-- GOOD: Security -->
  <title>{{I18N_LOGIN_TITLE}}</title>         <!-- GOOD: Early -->
...
<div class="loading-overlay" inert>...</div>
```
...
<head>
  <style>
    ...
    :root { --spacing-4: 1rem; --color-primary-500: #6366f1; }
    body { min-height: 100vh; display: flex; align-items: center; }
    .login-card { background: white; border-radius: 0.75rem; }
    .form-input { width: 100%; padding: 1rem; border: 1px solid #ddd; }
  </style>

...
describe("--install-extension", () => {
  it("should use EXTENSIONS_GALLERY when set", async () => {
    await expect(
      runCodeServerCommand(["--install-extension", "author.extension"], { EXTENSIONS_GALLERY: "{}" }),
...
const webCompatibleExtensions = extensions.filter((ext) => ext.manifest.browser && ext.targetPlatform.includes("web"))
```
